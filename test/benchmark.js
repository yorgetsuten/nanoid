...
bench.addEventListener('cycle', async ({ task }) => {
  let name = task.name.padEnd('async secure-random-string'.length)
  let hz = formatNumber(task.result.hz.toFixed(0)).padStart(10)

  if (task.name === 'uid') name = '\nNon-secure:\n' + name
